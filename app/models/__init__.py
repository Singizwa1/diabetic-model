...
class UserProfile(Base):
    """Static health profile for a user."""
...
class DailyLog(Base):
    """Daily symptom log during an assessment session."""
    ...
    urination_frequency = Column(Integer, nullable=False)  # times per day
    thirst_frequency = Column(Integer, nullable=False)  # times per day
    thirst_level = Column(Integer, nullable=False)  # 1-4 scale
    fatigue_level = Column(Integer, nullable=False)  # 1-5 scale
    physical_activity = Column(Boolean, nullable=False)  # yes/no
    alcohol_consumption = Column(Boolean, nullable=False)  # yes/no
    smoking = Column(Boolean, nullable=False)  # yes/no
    ...
    __table_args__ = (
        # Ensure unique logs per session per day
    )


class Prediction(Base):
    """Risk prediction result for a completed session."""
    ...
    feature_payload = Column(JSON, nullable=True)  # Store input features for explainability
    created_at = Column(DateTime, default=datetime.utcnow, index=True)
...
class Notification(Base):
    """In-app notifications (e.g., high risk alerts)."""
    ...
    notification_type = Column(String(50), nullable=False)  # "risk_alert", "session_complete", etc.
...
class PasswordResetRequest(Base):
    """Password reset request record stored before sending email."""
